...
/_/   \_\_|  |_|_|   |_____|_____\____|\___/|_|\__,_| (_) |_|\___/ 
    ...
    Forked from Ampleforth Token-Geyser: https://github.com/ampleforth/token-geyser/ and https://github.com/ampleforth/uFragments (Credits to Ampleforth team for implementation of rebasing on the ethereum network)
...
contract AMPLGSunshineReserve is IStaking, Ownable {
    ...
    constructor(IERC20 stakingToken, IERC20 distributionToken, uint256 maxUnlockSchedules,
        ...
        require(startBonus_ <= 10**BONUS_DECIMALS, 'Sunshine: start bonus too high');
        // If no period is desired, instead set startBonus = 100%
        // and bonusPeriod to a small value like 1sec.
        require(bonusPeriodSec_ != 0, 'Sunshine: bonus period is zero');
        require(initialSharesPerToken > 0, 'Sunshine: initialSharesPerToken is zero');
    ...
    function _stakeFor(address staker, address beneficiary, uint256 amount) private {
        require(amount > 0, 'Sunshine: stake amount is zero');
        require(beneficiary != address(0), 'Sunshine: beneficiary is zero address');
        require(totalStakingShares == 0 || totalStaked() > 0,
                'Sunshine: Invalid state. Staking shares exist, but no staking tokens do');

        uint256 mintedStakingShares = (totalStakingShares > 0)
            ? totalStakingShares.mul(amount).div(totalStaked())
            : amount.mul(_initialSharesPerToken);
        require(mintedStakingShares > 0, 'Sunshine: Stake amount is too small');
        ...
        require(_stakingPool.token().transferFrom(staker, address(_stakingPool), amount),
            'Sunshine: transfer into staking pool failed');
    ...
    function _unstake(uint256 amount) private returns (uint256) {
        ...
        // checks
        require(amount > 0, 'Sunshine: unstake amount is zero');
        require(totalStakedFor(msg.sender) >= amount,
            'Sunshine: unstake amount is greater than total user stakes');
        uint256 stakingSharesToBurn = totalStakingShares.mul(amount).div(totalStaked());
        require(stakingSharesToBurn > 0, 'Sunshine: Unable to unstake amount this small');
        ...
        require(_stakingPool.transfer(msg.sender, amount),
            'Sunshine: transfer out of staking pool failed');
        require(_unlockedPool.transfer(msg.sender, rewardAmount),
            'Sunshine: transfer out of unlocked pool failed');

        emit Unstaked(msg.sender, amount, totalStakedFor(msg.sender), "");
        emit TokensClaimed(msg.sender, rewardAmount);

        require(totalStakingShares == 0 || totalStaked() > 0,
                "Sunshine: Error unstaking. Staking shares exist, but no staking tokens do");
        return rewardAmount;
    }
    ...
    function lockTokens(uint256 amount, uint256 durationSec) external onlyOwner {
        require(unlockSchedules.length < _maxUnlockSchedules,
            'Sunshine: reached maximum unlock schedules');
        ...
        require(_lockedPool.token().transferFrom(msg.sender, address(_lockedPool), amount),
            'Sunshine: transfer into locked pool failed');
    ...
    function unlockTokens() public returns (uint256) {
        ...
        if (unlockedTokens > 0) {
            require(_lockedPool.transfer(address(_unlockedPool), unlockedTokens),
                'Sunshine: transfer out of locked pool failed');
